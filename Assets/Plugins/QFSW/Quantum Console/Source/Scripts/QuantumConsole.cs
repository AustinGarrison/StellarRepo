...
#pragma warning restore 0414, 0067, 0649
        ...
        protected virtual void Update()
        {
            if (!IsActive)
            {
                if (_keyConfig.ShowConsoleKey.IsPressed() || _keyConfig.ToggleConsoleVisibilityKey.IsPressed() || Input.GetKeyDown(KeyCode.F1))
                {
                    Activate();
                }
            }
            else
            {
                ...
                if (_keyConfig.HideConsoleKey.IsPressed() || _keyConfig.ToggleConsoleVisibilityKey.IsPressed() || Input.GetKeyDown(KeyCode.F1))
                {
                    Deactivate();
                    return;
                }
